    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            Object.keys(totalSuratPerBulanMasuk).forEach(month => {
                totalSuratDataMasuk[month - 1] = totalSuratPerBulanMasuk[
                month]; // Fill data at appropriate index
            });
                                ...
                                <tr>
                                    ...
                                    <th><label for="startDate"></label>
                                        ...
                                        <label for="endDate">-</label><input type="date" id="endDate"
                                            class="form-control d-inline-block" style="width: 40%;">
                                    </th>
                                    <th></th>
                                    <th></th>
                                </tr>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if (auth()->user()->role == 'super_admin' || auth()->user()->role == 'admin')
                                                    ...
                                                    <a href="{{ route('balas_surat', $item->suratmasuk_id) }}"
                                                        class="btn btn-primary btn-sm" title="Balas Surat">
                                                ...
                                                @if (auth()->user()->role == 'super_admin')
                                                    <form
                                                        action="{{ route('suratmasuk.destroy', $item->suratmasuk_id) }}"
                                                        method="POST" id="delete-form-{{ $item->suratmasuk_id }}"
                                                        style="display:inline;">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button" class="btn btn-danger btn-sm"
                                                            onclick="confirmDelete({{ $item->suratmasuk_id }})"><i
                    ...
                    <nav aria-label="Page navigation example">
                        <ul class="pagination">
                            ...
                            @if ($startPage > 1)
                                <li class="page-item">
                                    <a class="page-link"
                                        href="{{ $suratMasuk->url(1) }}&limit={{ $suratMasuk->perPage() }}">1</a>
                                </li>
                                @if ($startPage > 2)
                            ...
                            @for ($i = $startPage; $i <= $endPage; $i++)
                                <li class="page-item {{ $i == $currentPage ? 'active' : '' }}">
                                    <a class="page-link"
                                        href="{{ $suratMasuk->url($i) }}&limit={{ $suratMasuk->perPage() }}">{{ $i }}</a>
                                </li>
                            @endfor
                            ...
                            @if ($endPage < $lastPage)
                                ...
                                <li class="page-item">
                                    <a class="page-link"
                                        href="{{ $suratMasuk->url($lastPage) }}&limit={{ $suratMasuk->perPage() }}">{{ $lastPage }}</a>
                                </li>
                            @endif
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="card">
                                <div class="card-header">
                                    <div class="controls">
                                        <i class="fas fa-minus" onclick="toggleChart('chartIndexMasuk')"></i>
                                        <i class="fas fa-times" onclick="removeChart('chartIndexMasuk')"></i>
                                    </div>
                                    <span>Grafik Total Indeks Dipakai</span>
                                </div>
                                <div class="card-body">
                                    <canvas id="chartIndexMasuk"></canvas>
                                </div>
                            </div>
                        </div>
                    </div>
